...
>
    ...
    [ASN1PreparedElement]
    [ASN1Choice ( Name = "<xsl:value-of select='$elementName'/>", IsExtensible = <xsl:choose><xsl:when test="elementTypeList/isExtensible = 'true'">true</xsl:when><xsl:otherwise>false</xsl:otherwise></xsl:choose> )]
