...
>
    ...
    [ASN1PreparedElement]
    [ASN1Sequence ( Name = "<xsl:value-of select='$sequenceName'/>", IsExtensible = <xsl:choose><xsl:when test="elementTypeList/isExtensible = 'true'">true</xsl:when><xsl:otherwise>false</xsl:otherwise></xsl:choose>, IsSet = <xsl:choose><xsl:when test="isSequence = 'false'">true</xsl:when><xsl:otherwise>false</xsl:otherwise></xsl:choose>)]
