...
namespace org.bn.coders.per
{
    ...
    /// <summary> Encoding constraint length determinant procedure.
        ...
        /// 
        /// </summary>
        protected virtual int encodeConstraintLengthDeterminant(int length, ASN1ValueRangeConstraintMetadata constraint, BitArrayOutputStream stream)
        {
            int min = (int)constraint.Min;
            int max = (int)constraint.Max;
            bool isExtensible = constraint.IsExtensible;
            bool isExtended = constraint.checkIsExtended(length);

            if (!isExtensible && isExtended)
            {
                throw new ArgumentOutOfRangeException($"Length {length} is out of the allowed range [{min}, {max}] and the field is not extensible.");
            }

            if (isExtensible)
            {
                stream.writeBit(isExtended);
            }

            if (((max-min) <= 0xFFFF) && !(isExtensible && isExtended))
            {
		...
		protected virtual int encodeConstraintNumber(long val, long min, long max, BitArrayOutputStream stream)
		{
			...
			else
			{
				...
				*/
                ...
                ASN1ValueRangeConstraintMetadata metaConstraint = new(valueRangeConstraint);

                result = encodeConstraintLengthDeterminant(CoderUtils.getIntegerLength(narrowedVal), metaConstraint, stream);
				doAlign(stream);
		...
		public override int encodeInteger(object obj, System.IO.Stream stream, ElementInfo elementInfo)
		{
            ...
            if (obj.GetType().Equals(typeof(int)))
            {
                ...
                bool isExtended = (val < min) || (val > max);
                if (isExtensible)
                {
                    bitStream.writeBit(isExtended);
                }

                if (hasConstraint && isExtended && !isExtensible)
                {
                    throw new ArgumentOutOfRangeException($"Value {val} is out of the allowed range [{min}, {max}] and the field is not extensible.");
                }
        ...
        protected int encodeLength(int val, ElementInfo elementInfo, System.IO.Stream stream)
        {
            ...
            else
            {
                ...
                {
                    ASN1ValueRangeConstraint constraint = elementInfo.getAttribute<ASN1ValueRangeConstraint>();
                    resultSize += encodeConstraintLengthDeterminant(val, new ASN1ValueRangeConstraintMetadata(constraint), bitStream);
                }
                else
        ...
        public class ASN1SequenceFieldPresence {
            public List<bool> PreampleVector = new();
            ...
            foreach (PropertyInfo field in fields)
            {
                ...
                if (isOptionalField || isExtendeField)
                {
                    ...
                    else if (CoderUtils.isDefaultField(field, info))
                    {
                        ...
                        fieldResult = !CoderUtils.AreEqual(defaultFieldValue, invokeObjResult);
                    }
                    else
                    {
                        fieldResult = true;
                    ...
                    {
                        presence.PreampleVector.Add(fieldResult);
                    }
                }
                fieldIdx++;
            }
        ...
        public virtual int encodeSequencePreamble(ASN1SequenceFieldPresence presence, System.IO.Stream stream)
		{
			int resultBitSize = 0;

            foreach (var bit in presence.PreampleVector)
            {
                ((BitArrayOutputStream)stream).writeBit(bit);
                resultBitSize += 1;
            }
			doAlign(stream);
        ...
        public override int encodeSequence(object obj, System.IO.Stream stream, ElementInfo elementInfo)
		{
            ...
            {
                var presence = estimateFieldPresense(obj, elementInfo.getProperties(obj.GetType()), elementInfo);
                if (presence.IsExtensible)
                {
                    ((BitArrayOutputStream)stream).writeBit((presence.MaxExtensionLength > 0));
                }
                ...
                {
                    resultSize += encodeNormallySmallNumber(presence.MaxExtensionLength-1, (BitArrayOutputStream)stream);
                    // write the extension preamble
                    for (int extIdx=0; extIdx<presence.MaxExtensionLength; extIdx++)
                    {
        ...
        public virtual int encodeSet(object obj, System.IO.Stream stream, ElementInfo elementInfo)
        {
            ...
            ASN1SequenceFieldPresence presence = estimateFieldPresense(obj, fields, elementInfo);
