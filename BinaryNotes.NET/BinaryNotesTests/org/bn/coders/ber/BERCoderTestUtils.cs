...
namespace org.bn.coders.ber
{
	...
	public class BERCoderTestUtils:CoderTestUtilities
	{
        ...
        {
            return new byte[] {0x31, 0x10,
                (byte)0x81, 0x04, 0x61, 0x61, 0x61, 0x61,
                (byte)0x82, 0x02, 0x00, (byte)0xAA,
                (byte)0x83, 0x04, 0x62, 0x62, 0x62, 0x62};
        }

        public override byte[] createSetWithDefaultValueBytes()
        {
            return new byte[]{0x31, 0x0A,
                (byte) 0x81, 0x04, 0x61, 0x61, 0x61, 0x61,
                (byte) 0x82, 0x02, 0x00, (byte) 0xAA};
        }
        ...
        {
            return new byte[]{0x30, 0x04, (byte) 0x80, 0x02, 0x00, (byte) 0xAA};
        }
        ...
        public override byte[] createTestSequenceV12Bytes()
        {
            return new byte[] {0x30, 0x33,
                (byte)0x80, 0x03, 0x61, 0x62, 0x61,
                (byte)0x81, 0x04, 0x63, 0x63, 0x63, 0x63,
                (byte)0x82, 0x06, 0x64, 0x64, 0x64, 0x64, 0x64, 0x64, // attrStr2
                (byte)0xA3, 0x0B, 0x13, 0x04, 0x61, 0x61, 0x61, 0x61, 0x13, 0x03, 0x62, 0x62, 0x62, // attrArr
                (byte)0x84, 0x03, 0x01, (byte)0x99, (byte)0x80, // attrBitStr
                (byte)0x86, 0x02, 0x04, (byte)0xF0, // attrBitStrBnd
                (byte)0x87, 0x02, 0x04, (byte)0xA0, // attrBoxBitStr
                (byte)0x88, 0x04, 0x0A, 0x0B, 0x0C, 0x0D}; // attrStrict
        }
        ...
        public override byte[] createTestExtendedInteger1Bytes()
        {
            return new byte[] { 0x02, 0x01, 0x23 };
        }

        public override byte[] createTestExtendedInteger2Bytes()
        {
            return new byte[] { 0x02, 0x01, 0x73 };
        }

        public override byte[] createTestExtensibleSize1Bytes()
        {
            return new byte[] { 0x30, 0x06, 0x02, 0x01, 0x23, 0x02, 0x01, 0x45 };
        }

        public override byte[] createTestExtensibleSize2Bytes()
        {
            return new byte[] { 0x30, 0x0c, 0x02, 0x01, 0x23, 0x02, 0x01, 0x35, 0x02, 0x01, 0x11, 0x02, 0x01, 0x05 };
        }
    }
}
