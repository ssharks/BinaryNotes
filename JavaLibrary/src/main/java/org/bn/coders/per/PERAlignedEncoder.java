...
public class PERAlignedEncoder extends Encoder {
    ...
    /**
     * Encoding length determinant procedure. ITU-T X.691. 10.9. General rules
     * for encoding a length determinant
     */
    ...
    /**
     * Encoding of a constrained whole number ITU-T X.691. 10.5. NOTE �
    ...
    /**
     * Encoding of a semi-constrained whole number ITU-T X.691. 10.7. NOTE �
     ...
     * non-negative-binary-integer, and requires an explicit length encoding
    ...
    /**
     * Encode normally small number ITU-T X.691. 10.6 NOTE � (Tutorial) This
    ...
    /**
     * Encoding of a unconstrained whole number ITU-T X.691. 10.8. NOTE �
     * (Tutorial) This case only arises in the encoding of the value of an
     * integer type with no lower bound. The procedure encodes the value as a
     * 2's-complement-binary-integer into the minimum number of octets required
     * to accommodate the encoding, and requires an explicit length encoding
     * (typically a single octet) as specified in later procedures.
     */
    ...
    /**
     * Encoding of the choice structure ITU-T X.691. 22. NOTE � (Tutorial) A
     * choice type is encoded by encoding an index specifying the chosen
     * alternative. This is encoded as for a constrained integer (unless the
     * extension marker is present in the choice type, in which case it is a
     * normally small non-negative whole number) and would therefore typically
     * occupy a fixed length bit-field of the minimum number of bits needed to
     * encode the index. (Although it could in principle be arbitrarily large.)
     * This is followed by the encoding of the chosen alternative, with
     * alternatives that are extension additions encoded as if they were the
     * value of an open type field. Where the choice has only one alternative,
     * there is no encoding for the index.
     */
    @Override
