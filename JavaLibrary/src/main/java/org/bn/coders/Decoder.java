...
public abstract class Decoder implements IDecoder, IASN1TypesDecoder {
    ...
    public DecodedObject<?> decodeClassType(DecodedObject<Integer> decodedTag, Class<?> objectClass, ElementInfo elementInfo, InputStream stream) throws Exception {
        ...
        } else if (elementInfo.getAnnotatedClass().isAnnotationPresent(ASN1BitString.class)
                || elementInfo.getAnnotatedClass().equals(BitString.class)) {
            return decodeBitString(decodedTag, objectClass, elementInfo, stream);
        } else if (elementInfo.getAnnotatedClass().isAnnotationPresent(ASN1ObjectIdentifier.class)
    ...
    protected DecodedObject<?> decodeJavaElement(DecodedObject<Integer> decodedTag, Class objectClass, ElementInfo elementInfo, InputStream stream) throws Exception {
        if (elementInfo.getAnnotatedClass().equals(String.class)) {
            ...
            return decodeInteger(decodedTag, objectClass, elementInfo, stream);
        } else if (elementInfo.getAnnotatedClass().equals(Long.class)) {
            return decodeInteger(decodedTag, objectClass, elementInfo, stream);
        } else if (elementInfo.getAnnotatedClass().equals(Double.class)) {
            return decodeReal(decodedTag, objectClass, elementInfo, stream);
        } else if (elementInfo.getAnnotatedClass().equals(Boolean.class)) {
            return decodeBoolean(decodedTag, objectClass, elementInfo, stream);
        } else if (elementInfo.getAnnotatedClass().equals(byte[].class)) {
            return decodeOctetString(decodedTag, objectClass, elementInfo, stream);
        } else {
            return null;
        }
    ...
    @Override
    public <T> DecodedObject<T> decodeEnum(DecodedObject<Integer> decodedTag, Class<T> objectClass, ElementInfo elementInfo, InputStream stream) throws Exception {
